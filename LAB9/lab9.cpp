...
class PerCourse : public PayKind{
  public:
    ...
    PerCourse(unsigned userFee) {_feePerCourse = userFee;}
    ~PerCourse() {cout << "PerCourse destructor called" << endl;}
    double payAdvice()
    {
      double sal;
      sal = _feePerCourse / 7;
    ...
    unsigned getFee() const {return _feePerCourse;}
    void setFee(unsigned userNum) {_feePerCourse = userNum;}
...
class PartTime : public EmployeeKind{
  public:
    ...
    PartTime(unsigned userHours) {_maxHours = userHours;}
    ~PartTime() {cout << "PartTime destructor called" << endl;}

    void output(ostream& out) const { out << *this; }
				
	  friend ostream& operator<< (ostream&, const PartTime&);

    unsigned getHours() const {return _maxHours;}
    void setHours(unsigned userHours) {_maxHours = userHours;}
...
ostream& operator<< (ostream& out, const PerCourse& s) {
    out << "Course Rate:\t" << s.getFee() << endl;

    return out;
}

ostream& operator<< (ostream& out, const PartTime& s){
  out << "Max Hours:\t" << s.getHours() << endl;
...
class Adjunct : public Faculty, public PerCourse, public PartTime{
  public:
    ...
    Adjunct(unsigned userCourses) {_maxCourses = userCourses;}
    ~Adjunct() {cout << "Adjunct destructor called" << endl;}
Adjunct(string firstName, string lastName, string address, unsigned _IDNumber,
			string SSN, string department, unsigned maxCourses, unsigned feePerCourse, unsigned maxHours);
	    
    void output(ostream& out) const { out << *this; }
    friend ostream& operator<< (ostream&, const Adjunct&);

    unsigned getCourses() const {return _maxCourses;}
    void setCourses(unsigned userCourses) {_maxCourses = userCourses;}
...
ostream& operator<< (ostream& out, const Adjunct& s)
{
  s.Faculty::output(out);
  out << "Max Courses:\t" << s.getCourses() << endl;
  s.PerCourse::output(out);
  s.PartTime::output(out);
  return out;
}
...
int main (void) {
  ...
  cout << "Grader Faculty g1 Pay advice: \n" << g2.payAdvice() << endl << endl;
  ...
  cout << "NTT Faculty n1(Tom...) Pay advice: \n" << n1.payAdvice() << endl << endl;

 // Adjunct a1(4);
  //cout << "Adjunct a1(0): \n" << a1 << endl;


  Adjunct a2("Bob", "vi", "220 Pawtucket Street", 11223344, "987-65-4521", "Mechanical", 5, 30, 45);
  cout << "\n" << a2 << endl;
  std::vector<NTT*> nv;
  nv.push_back(&n1);
  cout << 0 << ":\n" << *nv[0];
  cout << "Pay Advice is " << nv[0]->payAdvice() << endl << endl;
     std::vector<UMLPerson*> uv;
     uv.push_back(&s1);
     uv.push_back(&s2);
     uv.push_back(&g1);
     uv.push_back(&n1);
  ...
  //Replacing NTT with UMLPerson will cause an error, as UMLPerson is a base class and does not have the payAdvice Function inhereted from payKind

  //To make the output of the for loop better, you would have to add more detail about the people. To do this you would edit the ostream << function for NTT to print more info, such as their job, payKind, and payAdvice.
