...
//	createUMLPeopleDB(); // not needed for Lab 11 - output is already in file lab11_input.txt
  ...
  stable_sort(upDB.begin(), upDB.end(), compare_IDs);
	...
	cout << "Using std::find:\n";
	// *** You write it!
  auto p = find(upDB.begin(), upDB.end(), 87654321);
  if (p != upDB.end())
  {
    cout << "IDNumber 87654321 found in upDB for UMLPerson: " << *p;
  }
  else
  {
    cout << "IDNumber 87654321 not found in upDB" << endl;
  }
  auto p2 = find(upDB.begin(), upDB.end(), 89283002);
  if (p2 != upDB.end())
  {
    cout << "IDNumber 89283002 found in upDB for UMLPerson: " << *p2;
  }
  else
  {
	...
	// *** You write it!
  auto q = minmax_element(upDB.begin(), upDB.end());
  cout << "min is " << *q.first;
  cout << "max is " << *q.second << endl;
	cout << "std::minmax_element in upDB using Compare function compare_IDs:\n";
	// *** You write it!
  auto r = minmax_element(upDB.begin(), upDB.end(), compare_IDs);
  cout << "min is " << *r.first;
  cout << "max is " << *r.second << endl;
