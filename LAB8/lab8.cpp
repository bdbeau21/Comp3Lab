...
class Adjunct : public Faculty, public PerCourse, public PartTime { // derived class
    public:
		// constructors and destructor
...
//	    Adjunct(const Faculty&); // copy constructor
    ...
    private:
        unsigned _maxCourses; // max courses of adjunct
};
...
ostream& operator<< (ostream& out, const Grader& u) {
  ...
  u.PartTime::output(out); // display the Max Hours variables
...
ostream& operator<< (ostream& out, const Adjunct& n) {
	...
	n.PerCourse::output(out); // display the PerCourse variables
  n.PartTime::output(out); // display the PartTime variables
...
int main (void) {
  ...
  Adjunct a1("Bob", "Chang", "220 Pawtucket Street", 11223344, "987-65-4321", "Computer Science", 5, 30, 45);
  cout << "Adjunct Faculty a1(Bob...):\n" << a1 << endl;
